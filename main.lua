...
function Library:CreateWindow(cfg)
	...
	function Window:SetTitle(text) if TitleLbl then TitleLbl.Text = text end end
	...
	function Window:CreateTab(tcfg)
		...
		function Tab:CreateDivider()
			...
			create("Frame", {
				...
				Size = UDim2.new(1, 0, 0, 1), BorderSizePixel = 0, Parent = row,
			})
			return { Instance = row }
		end

		------------------------------------------------------------
		-- 11. Section (small caps header used to group rows)
		------------------------------------------------------------
		function Tab:CreateSection(text)
			...
			local lbl = create("TextLabel", {
				BackgroundTransparency = 1, Text = string.upper(text or "Section"),
			...
			return { Set = function(_, t) lbl.Text = string.upper(t) end, Instance = row }
		end

		------------------------------------------------------------
		-- 12. Paragraph (title + wrapped body text)
		------------------------------------------------------------
		function Tab:CreateParagraph(pcfg)
			...
			local titleLbl = create("TextLabel", {
				BackgroundTransparency = 1, Text = pcfg.Title or "Paragraph",
				FontFace = FONT_TITLE, TextColor3 = Theme.Text, TextSize = 14,
				TextXAlignment = Enum.TextXAlignment.Left, TextWrapped = true,
			...
			return {
				Set = function(_, title, content)
					if title then titleLbl.Text = title end
					if content then bodyLbl.Text = content end
				end,
				Instance = row,
			}
		end
		...
		function Tab:CreateProgressBar(pcfg)
			pcfg = pcfg or {}
			local min, max = pcfg.Min or 0, pcfg.Max or 100
			local value = math.clamp(pcfg.Default or min, min, max)
			local row = newRow(40)

			create("TextLabel", {
				BackgroundTransparency = 1, Text = pcfg.Name or "Progress",
				FontFace = FONT_MAIN, TextColor3 = Theme.Text, TextSize = 14,
				TextXAlignment = Enum.TextXAlignment.Left, Position = UDim2.new(0, 10, 0, 5),
				Size = UDim2.new(0.6, 0, 0, 16), Parent = row,
			})
			local valLbl = create("TextLabel", {
				BackgroundTransparency = 1, Text = tostring(value) .. "/" .. tostring(max),
		...
		function Tab:CreateKeybind(kcfg)
			kcfg = kcfg or {}
			local bind = kcfg.Default or Enum.KeyCode.Unknown
			local listening = false
			local row = newRow(36)

			create("TextLabel", {
				BackgroundTransparency = 1, Text = kcfg.Name or "Keybind",
				FontFace = FONT_MAIN, TextColor3 = Theme.Text, TextSize = 14,
				TextXAlignment = Enum.TextXAlignment.Left, AnchorPoint = Vector2.new(0, 0.5),
			...
			function api:Set(kc)
				...
				if kcfg.Callback then task.spawn(kcfg.Callback, bind) end
			end
			...
			-- fires kcfg.OnPress whenever the bound key is pressed elsewhere in the game
			UserInputService.InputBegan:Connect(function(inp, gp)
				if gp or listening then return end
				if bind ~= Enum.KeyCode.Unknown and inp.KeyCode == bind and kcfg.OnPress then
		...
		function Tab:CreateImage(icfg)
			...
			Tab._order += 1
			local row = create("Frame", {
				BackgroundColor3 = Theme.Element, Size = UDim2.new(1, 0, 0, icfg.Height or 140),
				LayoutOrder = Tab._order, BorderSizePixel = 0, Parent = page,
			})
			corner(row, 6)
			stroke(row, Theme.Stroke, STROKE_T)
			local img = create("ImageLabel", {
				BackgroundTransparency = 1, Image = icfg.Image or "",
				ScaleType = Enum.ScaleType.Fit, Size = UDim2.new(1, -12, 1, -12),
		...
		------------------------------------------------------------
		-- 16. Button Group (row of side-by-side buttons)
		------------------------------------------------------------
		function Tab:CreateButtonGroup(bgcfg)
			bgcfg = bgcfg or {}
			local buttons = bgcfg.Buttons or {}
			local row = newRow(36)
			...
			for i, bcfg in buttons do
				local btnEl = create("TextButton", {
					Text = "", AutoButtonColor = false, BackgroundColor3 = Theme.Element,
					Size = UDim2.new(1 / #buttons, -4, 1, 0), LayoutOrder = i, BorderSizePixel = 0, Parent = row,
				})
				corner(btnEl, 5); stroke(btnEl, Theme.Stroke, STROKE_T)
				create("TextLabel", {
					BackgroundTransparency = 1, Text = bcfg.Name or ("Button " .. i),
					FontFace = FONT_MAIN, TextColor3 = Theme.Text, TextSize = 13,
					TextTruncate = Enum.TextTruncate.AtEnd, Size = UDim2.new(1, 0, 1, 0), Parent = btnEl,
				})
				btnEl.MouseEnter:Connect(function() tween(btnEl, TI, { BackgroundColor3 = Theme.ElementHover }) end)
				btnEl.MouseLeave:Connect(function() tween(btnEl, TI, { BackgroundColor3 = Theme.Element }) end)
				btnEl.Activated:Connect(function()
		...
		------------------------------------------------------------
		-- 17. Selector (cycle through options with arrows)
		------------------------------------------------------------
		function Tab:CreateSelector(secfg)
			secfg = secfg or {}
			local options = secfg.Options or {}
			local idx = 1
			for i, o in options do
				if o == secfg.Default then idx = i; break end
			end
			local row = newRow(36)

			create("TextLabel", {
				BackgroundTransparency = 1, Text = secfg.Name or "Selector",
				FontFace = FONT_MAIN, TextColor3 = Theme.Text, TextSize = 14,
				TextXAlignment = Enum.TextXAlignment.Left, AnchorPoint = Vector2.new(0, 0.5),
			...
			local function set(newIdx, fire)
				...
				if fire and secfg.Callback then task.spawn(secfg.Callback, options[idx]) end
			end
			leftBtn.Activated:Connect(function() set(idx - 1, true) end)
			rightBtn.Activated:Connect(function() set(idx + 1, true) end)
			function api:Set(v)
				for i, o in options do if o == v then set(i, true); return end end
			end
		...
		------------------------------------------------------------
		-- 18. Hyperlink (clickable row, copies a URL to clipboard)
		------------------------------------------------------------
		function Tab:CreateHyperlink(hcfg)
			...
			local btnEl = create("TextButton", { Text = "", BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0), Parent = row })
			create("TextLabel", {
				BackgroundTransparency = 1, Text = hcfg.Name or "Link",
				FontFace = FONT_MAIN, TextColor3 = Theme.Text, TextSize = 14,
				TextXAlignment = Enum.TextXAlignment.Left, AnchorPoint = Vector2.new(0, 0.5),
...
----------------------------------------------------------------------
-- Watermark (draggable FPS / ping / custom text overlay)
----------------------------------------------------------------------
function Library:SetWatermark(cfg)
	...
	corner(bar, 6)
	stroke(bar, Theme.Stroke, STROKE_T)
	...
	function api:SetText(t)
		cfg.Text = t
		if not conn then lbl.Text = t end
	end
